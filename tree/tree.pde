...
void plant(int numLeaves, float minLeafScale, float maxLeafScale) { line(width/2, 0, width/2, height); // the plant's stem
...
leaf();
popMatrix();
angle += random(0.96, 1.04)*PI; // alternate the side for each leaf
} }